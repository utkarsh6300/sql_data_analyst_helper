...
DML_STATEMENTS = {
    ...
    "MERGE": "MERGE INTO t1 USING t1 AS src ON t1.id = src.id WHEN MATCHED THEN DELETE",
    "COPY": "COPY t1 TO STDOUT",
    "SELECT INTO": "SELECT * INTO t2 FROM t1",
    "VALUES": "VALUES (1, 2)",
    "TRUNCATE": "TRUNCATE t1",
    # A leading CTE does not change the root node type: these still parse to
    # Insert/Update/Delete, not Select, so the read-only pass's root check
    # (not just its tree walk, which is what catches a write hidden *inside*
    # a CTE under a SELECT root -- see tests/unit/passes/test_readonly.py)
    # must reject them too.
    "WITH CTE INSERT": "WITH x AS (SELECT 1) INSERT INTO t1 SELECT * FROM x",
    "WITH CTE UPDATE": "WITH x AS (SELECT 1) UPDATE t1 SET c1 = 1 FROM x",
    "WITH CTE DELETE": "WITH x AS (SELECT 1) DELETE FROM t1 USING x",
}


@pytest.mark.parametrize(
    "sql", list(DML_STATEMENTS.values()), ids=list(DML_STATEMENTS.keys())
)
def test_dml_statement_never_reaches_resolution_or_authorization(sql, compiler, policy):
...
def test_a_genuine_select_is_not_swept_up_by_the_same_net(compiler, policy):
    # This whole sweep is only meaningful if the read-only gate is selective
    # -- otherwise "every non-SELECT statement is rejected" would be true of
    # a pass that rejects everything, SELECT included. Placed here because
    # SELECT INTO and VALUES above are the DML-adjacent statements a broken,
    # over-eager filter would most plausibly confuse with a real SELECT.
